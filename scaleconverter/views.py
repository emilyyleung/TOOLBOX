	...
	if request.method == 'POST':
		...
		try:
			if scale_form.is_valid():
				...
				in_original_drawing_scale = scale_form.cleaned_data['original_drawing_scale']
				...
				print(in_original_drawing_scale)
				...
				ratio = int(in_original_drawing_scale) * power

				new_ratio = str(ratio).split(".")[0]

				export_string = "ORIGINAL SCALE 1:" + str(in_original_drawing_scale) + " @ " + str(in_original_drawing_size) + " | NEW SCALE" + " 1:" + str(new_ratio) + " @ " + str(in_printed_drawing_size)

				print(export_string)

				out["original_scale"] = str(in_original_drawing_scale)
